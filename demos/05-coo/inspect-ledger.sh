#!/usr/bin/env bash
# Inspect the tamper-evident agent-action ledger: every state-changing action
# taken by an agent (COO levers, CFO period closes), hash-chained and immutable.
#
# Reads straight from Postgres in the kind cluster (no host DB driver needed) and
# runs the server-side chain verifier. The ledger is out of bounds for the agents
# themselves — this is an operator/auditor view.
#
#   demos/05-coo/inspect-ledger.sh              # full ledger + chain check
...
# The chain: each row's prev_hash must equal the prior row's entry_hash; the
# hashes are shown truncated so the linkage is legible at a glance.
q -P pager=off -c "
SELECT seq,
       ...
       COALESCE(effect->'effect'->>'batch_id',
                effect->>'reason',
                CASE WHEN effect ? 'roles_captured'
                     THEN 'roles='||(effect->>'roles_captured') END,
                '') AS detail,
