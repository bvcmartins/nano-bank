...
- **Run tests from the repo root** (`/home/bmartins/dev/nano-bank`) with the project venv active so `import csuite`, `import platform` (the package, shadowing stdlib within the venv path is avoided — the package dir is `platform/` and imported as `platform` only from repo root; if a stdlib `platform` clash appears, the tests still import via the package path since repo root is first on `sys.path`), `import cto`, `import operations`, `import coo` resolve.

> **Naming note:** the new MCP package directory is `platform/`. Python has a stdlib module also named `platform`. Because tests and containers run with the repo root at the front of `sys.path` (and the container `COPY platform /app/platform`), `import platform` resolves to our package there. Do **not** rename it — the spec fixes the name — but never `import platform` for the stdlib inside this package; we don't need the stdlib module anywhere here.
